...
// ClipboardList, Target, Zap, Diamond, Gift, PlayCircle, AlertTriangle // Added AlertTriangle
// } from 'lucide-react';
...
// import SequentialItemDisplay from './SequentialItemDisplay';
// import SectionTitleBanner from './SectionTitleBanner';
// // Helper function to get RGB from Tailwind color names
...
//     const CAR_LOGICAL_Y = GROUND_Y_POSITION;
//     const CAR_WIDTH = 80;
...
//     // --- Game State ---
//     const [position, setPosition] = useState({ x: 50, y: CAR_LOGICAL_Y });
...
//     // --- State for Project Slideshow ---
//     const [projectImageIndex, setProjectImageIndex] = useState(0);
//     const [showProjectSlideshow, setShowProjectSlideshow] = useState(false);
    
//     // --- State for Section Warning ---
//     const [approachingSectionKey, setApproachingSectionKey] = useState(null); // Key of the upcoming section
//     const [showSectionWarning, setShowSectionWarning] = useState(false); // Visibility flag
...
//     const maxVisibleIndexPerSectionRef = useRef({}); // Track max reveal per section
//     const currentSectionKeyRef = useRef(currentSectionKey);
//     const projectSlideshowIntervalRef = useRef(null); // Ref for slideshow interval ID
    
//     // --- Portfolio Data ---
//     // NOTE: Project imageUrl paths assume images are DIRECTLY in /public/
//     const sections = {
//         'intro': {
//             title: 'Welcome!', icon: <PlayCircle size={20} className="text-teal-300" />,
//             contentTitle: "Aadarsh Raj", contentSubtitle: "Interactive Portfolio", contentAction: "Drive → to Explore",
//         },
//         'about': {
//             title: 'About Me', icon: <User size={20} className="text-sky-300" />,
//             content: 'MERN Stack | Generative AI | Game Dev (Unity/C#). Building seamless user experiences and innovative applications.',
//         },
//         'skills': { title: 'Skills', icon: <Code size={20} className="text-green-300" />, type: 'list', items: [ { id: 'sk1', label: 'React/Next.js', icon: <Zap size={18} className="text-sky-400"/> }, { id: 'sk2', label: 'Node/Express', icon: <Zap size={18} className="text-green-400"/> }, { id: 'sk3', label: 'MongoDB', icon: <Zap size={18} className="text-lime-400"/> }, { id: 'sk4', label: 'JS/TS', icon: <Zap size={18} className="text-yellow-400"/> }, { id: 'sk5', label: 'Python/PyTorch', icon: <Zap size={18} className="text-blue-400"/> }, { id: 'sk6', label: 'Generative AI', icon: <Zap size={18} className="text-purple-400"/> }, { id: 'sk7', label: 'Tailwind CSS', icon: <Zap size={18} className="text-teal-400"/> }, { id: 'sk8', label: 'AWS/DevOps', icon: <Zap size={18} className="text-orange-400"/> }, { id: 'sk9', label: 'Unity/C#', icon: <Zap size={18} className="text-gray-400"/> }, { id: 'sk10', label: 'Git', icon: <Zap size={18} className="text-red-400"/> } ] },
//         'experience': { title: 'Experience', icon: <ClipboardList size={20} className="text-indigo-300" />, type: 'list', items: [ { id: 'ex1', label: 'Freelancing', description: '1 Yr Full-Stack & AI solutions', icon: <Gift size={18} className="text-indigo-400"/> }, { id: 'ex2', label: 'Game Dev Intern', description: 'Unity/C# @ Kolkata Co.', icon: <Gift size={18} className="text-violet-400"/> } ] },
//         'projects': {
//             title: 'Projects', icon: <Briefcase size={20} className="text-amber-300" />, type: 'list',
//             items: [
//                 // !! ENSURE these images exist DIRECTLY in /public/ !!
//                 { id: 'p1', label: 'SanskritAI', description: 'AI Tutor (5k+ Users)', icon: <Target size={18} className="text-amber-400"/>, imageUrl: '/sanskritai.jpeg' },
//                 { id: 'p2', label: 'DivineWall', description: 'AI Wallpapers (8k+ Users)', icon: <Target size={18} className="text-orange-400"/>, imageUrl: '/divinewall.jpeg' },
//                 { id: 'p3', label: 'MyResearchPaper', description: 'AI Paper Generator', icon: <Target size={18} className="text-yellow-400"/>, imageUrl: '/myresearchpaper.jpeg' },
//                 { id: 'p4', label: 'Zombie Alien Game', description: 'Mobile Game (25k+ Players)', icon: <Target size={18} className="text-lime-400"/>, imageUrl: '/zombiegame.jpg' },
//                 { id: 'p5', label: 'Auto Express Game', description: 'Mobile Game', icon: <Target size={18} className="text-green-400"/>, imageUrl: '/autoexpress.jpg' }
//             ]
//          },
//         'awards': { title: 'Achievements', icon: <Award size={20} className="text-yellow-300" />, type: 'list', items: [ { id: 'aw1', label: '3rd Nat. Science Exbn.', description: '(Prahar Device)', icon: <Award size={18} className="text-yellow-400"/> }, { id: 'aw2', label: '2nd State Science Exbn.', description: '', icon: <Award size={18} className="text-amber-400"/> }, { id: 'aw3', label: "11th Google Code-in '19", description: '(vs 10k+)', icon: <Award size={18} className="text-orange-400"/> }, { id: 'aw4', label: 'Built Farmer Drone', description: '& other devices', icon: <Award size={18} className="text-red-400"/> } ] },
//         'education': { title: 'Education', icon: <GraduationCap size={20} className="text-rose-300" />, type: 'list', items: [ { id: 'ed1', label: 'B.Tech CSE @ LPU', description: '(2023-2026)', icon: <GraduationCap size={18} className="text-rose-400"/> }, { id: 'ed2', label: 'Diploma CSE @ LPU', description: '(2020-2023)', icon: <GraduationCap size={18} className="text-pink-400"/> }, { id: 'ed3', label: '10th - A.I.S.', description: '(2020)', icon: <GraduationCap size={18} className="text-fuchsia-400"/> } ] },
//         'contact': { title: 'Get In Touch', icon: <Mail size={20} className="text-purple-300" />, content: 'aadarshraj1516@gmail.com | LinkedIn: /in/aadarsh-raj-dev' },
//     };
//     const sectionKeys = Object.keys(sections);
//     const numSections = sectionKeys.length;
//     // Define which sections trigger a warning (Customize this list)
//     const sectionsToWarnAbout = ['skills', 'experience', 'projects', 'awards', 'education', 'contact'];
...
//      // --- Day/Night Cycle ---
//     useEffect(() => { const interval = setInterval(() => { setIsDayTime(prev => !prev); createParticles(15, !isDayTime ? 'night' : 'day'); }, 5000); return () => clearInterval(interval); }, [isDayTime, createParticles]);
...
//         const currentlyOnGround = currentPosition.y >= CAR_LOGICAL_Y; setIsOnGround(currentlyOnGround); const isTryingToJump = keys['arrowup'] || keys[' '];
...
//         // --- Ground Collision ---
//         if (nextY > CAR_LOGICAL_Y) { nextY = CAR_LOGICAL_Y; currentSpeed.y = 0; }
    
//          // --- Simple Coin Generation ---
//         if (Math.abs(currentSpeed.x) > 10 && Math.random() < 0.01) { setCoinCount(prev => prev + 1); createParticles(2, 'coin', { pos: { x: positionRef.current.x, y: positionRef.current.y - 10 } }); }
    
//         // --- Screen Boundaries & Background Scrolling ---
...
//         const newSectionKey = sectionKeys[currentSectionIndex % numSections]; // Current section key
    
//         const progressInSection = absoluteScroll % SECTION_WIDTH; // Progress within the current section
    
//         // --- Section Warning Logic ---
//         let nextApproachingKey = null; // Use temporary variables inside loop
...
//         // --- Sequential Item Logic ---
//         let newVisibleCount = 0;
//         const sectionData = sections[newSectionKey]; // Data for the section we are *currently* in
...
//         // Note: State setters (like setApproachingSectionKey) don't need to be dependencies for useCallback
//     ]);
    
//     // --- Effect to Start/Stop the Game Loop ---
//     useEffect(() => { lastTimeRef.current = performance.now(); requestRef.current = requestAnimationFrame(gameLoop); return () => { if (requestRef.current) { cancelAnimationFrame(requestRef.current); } }; }, [gameLoop]);
    
//     // --- Effect for Project Slideshow ---
//     useEffect(() => {
...
//     const showIdleAnimation = isOnGround && Math.abs(speed.x) < 5;
//     const currentSectionData = sections[currentSectionKey];
    
//     // --- Animation Variants ---
//     const aboutOverlayVariants = { hidden: { opacity: 0, scale: 0.9 }, visible: { opacity: 1, scale: 1, transition: { duration: 0.5, ease: "easeOut" } }, exit: { opacity: 0, scale: 0.95, transition: { duration: 0.3, ease: "easeIn" } } };
//     const aboutTextVariants = { hidden: { opacity: 0, y: 20 }, visible: { opacity: 1, y: 0, transition: { delay: 0.2, duration: 0.4, ease: "easeOut" } }, exit: { opacity: 0, y: -10, transition: { duration: 0.2, ease: "easeIn" } } };
...
//         hidden: { opacity: 0, scale: 0.5, y: 50, rotate: -5 },
//         visible: { opacity: 1, scale: 1, y: 0, rotate: 0, transition: { type: 'spring', stiffness: 150, damping: 15, duration: 0.4 } },
...
//             {/* --- Background Layers (Z: 0-30) --- */}
//              <div className={`absolute inset-0 transition-colors duration-[7000ms] ease-in-out z-0 ${getSkyColor()}`}> {/* Sky & Sun/Moon */}
...
//              </div>
//              <div className="absolute inset-x-0 top-[10%] h-20 pointer-events-none z-10 opacity-30" style={{ transform: `translateX(${backgroundPosition * 0.1}px)` }}> {/* Clouds */}
...
//              </div>
//              <div className="absolute inset-0 z-15 pointer-events-none"> {/* Particles */} {particles.map((p) => (<div key={`particle-${p.id ?? Math.random()}`} className="absolute rounded-full" style={{ left: p.x, top: p.y, width: Math.max(0, p.size), height: Math.max(0, p.size), backgroundColor: p.color, opacity: p.currentOpacity }} />))} </div>
//              <div className="absolute inset-0 z-25 pointer-events-none overflow-hidden"> {/* Project Slideshow */}
...
//                 </AnimatePresence>
//             </div>
//             <div className="absolute left-0 bottom-0 w-[6000px] h-1/3 pointer-events-none z-20" style={{ transform: `translateX(${backgroundPosition * 0.5}px)` }}> {/* Hills */}
...
//              </div>
//              <div className={`absolute bottom-0 w-full h-20 ${getGroundColor()} z-30 border-t-4 border-black/20`} /> {/* Ground */}
    
    
//             {/* --- Foreground Elements (Z: 35-50) --- */}
//              <div className="absolute left-0 bottom-0 w-[10000px] z-35 pointer-events-none" style={{ transform: `translateX(${backgroundPosition}px)` }}> {/* Ramps */}
//                  <div className="absolute" style={{ left: '1200px', bottom: '0px', width: '120px', height: '40px', background: 'linear-gradient(45deg, #4a5568, #2d3748)', clipPath: 'polygon(0% 100%, 50% 0%, 100% 100%)', transform: `translateY(-${80 - 12}px)`}}></div>
//                  <div className="absolute" style={{ left: '2800px', bottom: '0px', width: '150px', height: '50px', background: 'linear-gradient(45deg, #4a5568, #2d3748)', clipPath: 'polygon(0% 100%, 50% 0%, 100% 100%)', transform: `translateY(-${80 - 12}px)`}}></div>
//              </div>
//              <div className="absolute bottom-0 w-full h-12 bg-gray-800 z-40 border-t-2 border-gray-900 shadow-inner"> {/* Road */} {Array(20).fill(0).map((_, i) => { const lineSpacing = 150; const trackLength = 20 * lineSpacing; const linePos = (((i * lineSpacing) + backgroundPosition) % trackLength + trackLength) % trackLength; return (<div key={`roadline-${i}`} className="absolute h-1.5 w-12 bg-yellow-500 rounded-sm" style={{ left: `${linePos}px`, bottom: '8px' }} />); })} </div>
//              <div className={`absolute w-20 z-50 transition-transform duration-75 ease-out ${showIdleAnimation ? 'animate-carIdleBounce' : ''}`} style={{ left: position.x, top: position.y - CAR_VISUAL_HEIGHT, height: `${CAR_VISUAL_HEIGHT}px`, transform: `rotate(${Math.max(-20, Math.min(20, speed.y / 15))}deg)` }}> {/* Car */} <div className="absolute bottom-3 left-0 w-full h-10 bg-gradient-to-b from-red-600 to-red-700 rounded-t-md shadow-lg"/> <div className="absolute bottom-10 left-[15%] w-[70%] h-6 bg-gradient-to-b from-red-500 to-red-600 rounded-t-md shadow-inner"/> <div className="absolute bottom-[44px] left-[25%] w-[50%] h-4 bg-gradient-to-b from-sky-300 to-sky-500 opacity-80 rounded-t-sm border border-black/30"/> <div className="absolute bottom-0 left-2 w-6 h-6 bg-gray-900 rounded-full border-2 border-gray-700 flex items-center justify-center shadow-md" style={{ transform: `rotate(${(position.x - backgroundPosition) * 2.5}deg)` }}> <div className="w-2 h-2 bg-gray-500 rounded-full border border-gray-400"></div> </div> <div className="absolute bottom-0 right-2 w-6 h-6 bg-gray-900 rounded-full border-2 border-gray-700 flex items-center justify-center shadow-md" style={{ transform: `rotate(${(position.x - backgroundPosition) * 2.5}deg)` }}> <div className="w-2 h-2 bg-gray-500 rounded-full border border-gray-400"></div> </div> </div>
    
    
//             {/* --- Special Overlays (Z: 55) --- */}
//             <AnimatePresence>
...
//                         <motion.p variants={welcomeActionVariants} className="text-lg sm:text-xl text-yellow-400 font-medium animate-pulse">{currentSectionData.contentAction || "Drive → to Explore"}</motion.p>
//                     </motion.div>
//                 )}
...
//                         <div className="absolute inset-0 bg-contain bg-center bg-no-repeat opacity-30" style={{ backgroundImage: `url('/my-photo.jpeg')` }} /> {/* Your photo */}
//                         <div className="absolute inset-0 bg-black/70"></div>
//                         <motion.div variants={aboutTextVariants} className="relative max-w-3xl mx-auto p-8 text-center">
//                              <h3 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4 text-shadow-md">{currentSectionData.title || "About Me"}</h3>
//                             <p className="text-md md:text-lg lg:text-xl text-gray-200 leading-relaxed text-shadow-sm">{currentSectionData.content}</p>
//                         </motion.div>
//                     </motion.div>
//                 )}
//             </AnimatePresence>
...
//              {/* --- Section Approaching Warning (Z: 58) --- */}
...
//             {/* --- UI Layer (z-60) --- */}
//              {currentSectionKey !== 'intro' && currentSectionKey !== 'about' && currentSectionKey !== 'projects' && ( // Hide Banner for special sections
...
//             {currentSectionData?.type === 'list' && ( // Sequential Item Display
...
//             <div className="absolute top-5 right-5 flex flex-col items-end gap-2 z-60"> {/* Top Right UI */}
...
//             {currentSectionData?.content && currentSectionKey !== 'intro' && currentSectionKey !== 'about' && ( // Bottom Left Content
//                  <div className="absolute bottom-5 left-5 max-w-sm bg-black/70 backdrop-blur-sm p-3 rounded-lg text-white shadow-lg z-60 border border-white/10 text-xs">
//                      <p className="whitespace-pre-wrap">{currentSectionData.content}</p>
//                  </div>
//             )}
...
//             title: 'About Me', icon: <User size={20} className="text-sky-300" />,
//             // ** MORE DATA ADDED **
//             content: `Driven Full-Stack Developer with a deep passion for crafting intuitive user experiences and exploring the frontiers of Generative AI. Proficient in the MERN stack (MongoDB, Express, React, Node.js) and Python (especially PyTorch for AI), I thrive on building robust, scalable applications. My experience extends to Game Development using Unity/C#, allowing me to blend technical skills with creative problem-solving. I'm fascinated by AI ethics, innovative game mechanics, and building tools that genuinely help people. Eager to collaborate on challenging projects and contribute to impactful technology. Let's build something amazing together!`,
//         },
//         'skills': {
//             title: 'Skills', icon: <Code size={20} className="text-green-300" />, type: 'list',
//             // ** MORE DATA & CATEGORIZATION HINTS ADDED **
//             items: [
//                 // Frontend
...
//         },
//         'experience': {
//             title: 'Experience', icon: <ClipboardList size={20} className="text-indigo-300" />, type: 'list',
//             items: [
//                 { id: 'ex1', label: 'Freelance Developer', description: '1+ Yr: Full-Stack Web & AI Solutions for various clients.', icon: <Gift size={18} className="text-indigo-400"/> },
//                 { id: 'ex2', label: 'Game Dev Intern', description: 'Unity/C# @ Kolkata-based Co. (Contributed to mobile game logic & UI).', icon: <Gift size={18} className="text-violet-400"/> }
//             ]
//         },
//         'projects': {
//             title: 'Projects', icon: <Briefcase size={20} className="text-amber-300" />, type: 'list',
...
//          },
//         'awards': {
//             title: 'Achievements', icon: <Award size={20} className="text-yellow-300" />, type: 'list',
//             items: [
//                 { id: 'aw1', label: '3rd National Science Exbn.', description: 'Project Prahar (Security Device)', icon: <Award size={18} className="text-yellow-400"/> },
//                 { id: 'aw2', label: '2nd State Science Exbn.', description: '(Represented School/District)', icon: <Award size={18} className="text-amber-400"/> },
//                 { id: 'aw3', label: "Top 11th in Google Code-in '19", description: 'International Competition (vs 10k+)', icon: <Award size={18} className="text-orange-400"/> },
//                 { id: 'aw4', label: 'Built Farmer Assistance Drone', description: 'Also developed other IoT devices', icon: <Award size={18} className="text-red-400"/> }
//             ]
//         },
//         // ** NEW SECTION ADDED **
//         'certifications': {
...
//                 { id: 'ce2', label: 'Python for Everybody', description: 'Coursera (Specialization, 2022)', icon: <BadgeCheck size={18} className="text-sky-400"/> },
//                 { id: 'ce3', label: 'AI For Everyone', description: 'Coursera (DeepLearning.AI, 2023)', icon: <BadgeCheck size={18} className="text-blue-400"/> },
//                 { id: 'ce4', label: 'Google IT Support', description: 'Coursera (Professional Cert, 2021)', icon: <BadgeCheck size={18} className="text-teal-400"/> },
//                 // Add more relevant certifications here
//             ]
//         },
//         'education': {
...
//             // ** MORE DATA ADDED **
//             items: [
//                 { id: 'ed1', label: 'B.Tech CSE', description: 'Lovely Professional University (2023-2026). Focus on AI & Software Engg.', icon: <BookOpen size={18} className="text-rose-400"/> }, // Changed icon
//                 { id: 'ed2', label: 'Diploma CSE', description: 'Lovely Professional University (2020-2023). CGPA: 8.5+. Key Courses: DSA, DBMS, OS.', icon: <GraduationCap size={18} className="text-pink-400"/> },
//                 { id: 'ed3', label: 'Matriculation (10th)', description: 'Angel High School (CBSE Board, 2020). ', icon: <GraduationCap size={18} className="text-fuchsia-400"/> }
//             ]
//         },
//         'contact': {
//             title: 'Get In Touch', icon: <Mail size={20} className="text-purple-300" />,
//             content: 'Email: aadarshraj1516@gmail.com\nLinkedIn: /in/aadarsh-raj-dev\n(Links are not clickable in this format)', // Added line break for clarity
//         },
//     };

//     const sectionKeys = Object.keys(sections);
//     const numSections = sectionKeys.length;
//     // Define which sections trigger a warning (UPDATED)
//     const sectionsToWarnAbout = ['skills', 'experience', 'projects', 'awards', 'certifications', 'education', 'contact']; // Added 'certifications'

//     // --- Sync State to Refs --- (remains the same)
//     useEffect(() => { positionRef.current = position; }, [position]);
//     useEffect(() => { speedRef.current = speed; }, [speed]);
//     useEffect(() => { backgroundPositionRef.current = backgroundPosition; }, [backgroundPosition]);
//     useEffect(() => { currentSectionKeyRef.current = currentSectionKey; }, [currentSectionKey]);
...
//     // --- Particle System Logic --- (remains the same)
//     const createParticles = useCallback((count, type, options = {}) => {
//         const newParticles = []; const currentPos = options.pos || positionRef.current; const baseColor = options.color || [180, 180, 180]; const lifeMultiplier = options.life || 1; const speedMultiplier = options.speed || 1; const sizeMultiplier = options.size || 1;
//         for (let i = 0; i < count; i++) {
//              if (type === 'exhaust') { newParticles.push({ id: uuidv4(), x: currentPos.x - 5, y: currentPos.y - 5, size: (Math.random() * 4 + 2) * sizeMultiplier, color: `rgba(${baseColor[0]}, ${baseColor[1]}, ${baseColor[2]}, ${Math.random() * 0.4 + 0.1})`, vx: (-Math.random() * 30 - speedRef.current.x * 0.5 - 20) * speedMultiplier, vy: (Math.random() * 10 - 5) * speedMultiplier, life: (Math.random() * 0.5 + 0.2) * lifeMultiplier, type: 'exhaust' }); }
...
//         }
//         setParticles(prev => [...prev.slice(-200), ...newParticles]);
//     }, []); // Empty dependency array is okay if positionRef/speedRef are stable refs
...
//      // --- Day/Night Cycle --- (remains the same)
//     useEffect(() => { const interval = setInterval(() => { setIsDayTime(prev => !prev); createParticles(15, !isDayTime ? 'night' : 'day'); }, 5000); return () => clearInterval(interval); }, [isDayTime, createParticles]);

//     // --- Main Game Loop --- (logic updated slightly due to sectionsToWarnAbout change, but core is same)
//     const gameLoop = useCallback((timestamp) => {
//         const now = timestamp || performance.now(); const deltaTime = (now - lastTimeRef.current) / 1000; lastTimeRef.current = now; const dt = Math.min(deltaTime, 0.05);
//         let currentSpeed = { ...speedRef.current }; let currentPosition = { ...positionRef.current }; let currentBackgroundPos = backgroundPositionRef.current; const keys = keysPressedRef.current;

//         // --- Movement Physics --- (remains the same)
//         const isMovingRight = keys['arrowright'] || keys['d']; const isMovingLeft = keys['arrowleft'] || keys['a'];
//         if (isMovingRight) { currentSpeed.x = Math.min(currentSpeed.x + ACCELERATION, MAX_SPEED); } else if (isMovingLeft) { currentSpeed.x = Math.max(currentSpeed.x - ACCELERATION, -MAX_SPEED); } else { currentSpeed.x *= DECELERATION_FACTOR; if (Math.abs(currentSpeed.x) < 1) currentSpeed.x = 0; }
//         const currentlyOnGround = currentPosition.y >= CAR_LOGICAL_Y; setIsOnGround(currentlyOnGround); const isTryingToJump = keys['arrowup'] || keys[' '];
//         if (!currentlyOnGround || currentSpeed.y < 0) { currentSpeed.y += GRAVITY * dt; } if (isTryingToJump && currentlyOnGround) { currentSpeed.y = JUMP_VELOCITY; }

//         // --- Calculate Next Position --- (remains the same)
//         let nextX = currentPosition.x + currentSpeed.x * dt; let nextY = currentPosition.y + currentSpeed.y * dt;

//         // --- Ground Collision --- (remains the same)
//         if (nextY > CAR_LOGICAL_Y) { nextY = CAR_LOGICAL_Y; currentSpeed.y = 0; }

//          // --- Simple Coin Generation --- (remains the same)
//         if (Math.abs(currentSpeed.x) > 10 && Math.random() < 0.01) { setCoinCount(prev => prev + 1); createParticles(2, 'coin', { pos: { x: positionRef.current.x, y: positionRef.current.y - 10 } }); }

//         // --- Screen Boundaries & Background Scrolling --- (remains the same)
//         if (nextX < LEFT_BOUNDARY) { nextX = LEFT_BOUNDARY; currentSpeed.x = 0; }
//         else if (nextX > SCROLL_THRESHOLD && currentSpeed.x > 0) { const scrollAmount = currentSpeed.x * dt; currentBackgroundPos -= scrollAmount; nextX = SCROLL_THRESHOLD; }
//         else if (nextX < SCROLL_THRESHOLD && currentSpeed.x < 0 && currentBackgroundPos < 0) { const scrollAmount = currentSpeed.x * dt; currentBackgroundPos = Math.min(0, currentBackgroundPos - scrollAmount); }

//         // --- Final State Updates --- (remains the same)
//         setPosition({ x: nextX, y: nextY });
//         setSpeed(currentSpeed);
//         setBackgroundPosition(currentBackgroundPos);

//         // --- Section Detection & Logic --- (remains the same logic flow)
//         const totalTrackWidth = SECTION_WIDTH * numSections;
//         const absoluteScroll = Math.abs(currentBackgroundPos);
//         const currentSectionIndex = Math.floor(absoluteScroll / SECTION_WIDTH);
//         const newSectionKey = sectionKeys[currentSectionIndex % numSections]; // Current section key
...
//         // --- Section Warning Logic --- (remains the same logic, uses updated sectionsToWarnAbout)
...
//         if (progressInSection > (SECTION_WIDTH - SECTION_WARNING_THRESHOLD) && nextSectionIndex < sectionKeys.length) {
//             const potentialNextKey = sectionKeys[nextSectionIndex % numSections];
//             // Uses the updated `sectionsToWarnAbout` array here
//             if (sectionsToWarnAbout.includes(potentialNextKey)) {
//                  nextApproachingKey = potentialNextKey;
//                  nextShowWarning = true;
//             }
...
//         // --- Sequential Item Logic --- (remains the same logic flow)
//         let newVisibleCount = 0;
...
//         // --- Update Particles --- (remains the same)
//         updateParticles(dt); if (Math.random() < 0.1 && Math.abs(currentSpeed.x) > 50) { createParticles(1, 'exhaust'); }
...
//     // --- Effect to Start/Stop the Game Loop --- (remains the same)
//     useEffect(() => { lastTimeRef.current = performance.now(); requestRef.current = requestAnimationFrame(gameLoop); return () => { if (requestRef.current) { cancelAnimationFrame(requestRef.current); } }; }, [gameLoop]);

//     // --- Effect for Project Slideshow --- (remains the same)
//     useEffect(() => {
//         const isProjectsSection = currentSectionKey === 'projects';
//         const projectItems = sections.projects.items;
//         setShowProjectSlideshow(isProjectsSection);
...
//         }
//         return () => { if (projectSlideshowIntervalRef.current) { clearInterval(projectSlideshowIntervalRef.current); } };
//     }, [currentSectionKey]); // Dependency array remains the same
...
//     // --- Render --- (structure remains the same, logic updates flow through)
//     return (
//         <div className="w-full h-screen overflow-hidden relative bg-black focus:outline-none select-none" tabIndex={0}>
...
//             {/* --- Foreground Elements (Z: 35-50) --- (No changes here) */}
//              <div className="absolute left-0 bottom-0 w-[10000px] z-35 pointer-events-none" style={{ transform: `translateX(${backgroundPosition}px)` }}> {/* Ramps/Scenery - Add more if desired */}
//                  {/* Example Ramp: */}
//                  {/* <div className="absolute" style={{ left: '1200px', bottom: '0px', width: '120px', height: '40px', background: 'linear-gradient(45deg, #4a5568, #2d3748)', clipPath: 'polygon(0% 100%, 50% 0%, 100% 100%)', transform: `translateY(-${80 - 12}px)`}}></div> */}
//              </div>
//              <div className="absolute bottom-0 w-full h-12 bg-gray-800 z-40 border-t-2 border-gray-900 shadow-inner"> {/* Road */} {Array(20).fill(0).map((_, i) => { const lineSpacing = 150; const trackLength = 20 * lineSpacing; const linePos = (((i * lineSpacing) + backgroundPosition) % trackLength + trackLength) % trackLength; return (<div key={`roadline-${i}`} className="absolute h-1.5 w-12 bg-yellow-500 rounded-sm" style={{ left: `${linePos}px`, bottom: '8px' }} />); })} </div>
...
//             {/* --- Special Overlays (Z: 55) --- (Updated 'about' content will show here) */}
//             <AnimatePresence>
//                 {currentSectionKey === 'intro' && (
//                     <motion.div key="welcome-overlay" variants={welcomeOverlayVariants} initial="hidden" animate="visible" exit="exit" className="absolute inset-0 z-55 flex flex-col items-center justify-center text-center pointer-events-none p-4">
//                          <div className="absolute inset-0 bg-gradient-radial from-teal-500/10 via-transparent to-transparent blur-3xl opacity-50"></div>
...
//                 {currentSectionKey === 'about' && (
//                     <motion.div key="about-me-overlay" variants={aboutOverlayVariants} initial="hidden" animate="visible" exit="exit" className="absolute inset-0 z-55 flex items-center justify-center pointer-events-none">
//                          {/* Optional: Add a subtle background image or pattern if desired */}
//                          {/* <div className="absolute inset-0 bg-contain bg-center bg-no-repeat opacity-10" style={{ backgroundImage: `url('/your-subtle-background.png')` }} /> */}
//                         <div className="absolute inset-0 bg-black/75 backdrop-blur-sm"></div> {/* Adjusted overlay */}
//                         <motion.div variants={aboutTextVariants} className="relative max-w-3xl mx-auto p-8 text-center">
//                              <h3 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4 text-shadow-md">{currentSectionData.title || "About Me"}</h3>
//                              {/* Content is now longer due to updates */}
//                             <p className="text-md md:text-lg lg:text-xl text-gray-200 leading-relaxed text-shadow-sm whitespace-pre-wrap">{currentSectionData.content}</p>
//                         </motion.div>
//                     </motion.div>
//                 )}
//             </AnimatePresence>

//              {/* --- Section Approaching Warning (Z: 58) --- (No changes here, uses updated data) */}
//             <div className="absolute inset-0 z-58 pointer-events-none flex items-center justify-center">
//                 <AnimatePresence>
//                     {showSectionWarning && approachingSectionKey && sections[approachingSectionKey] && (
//                         <motion.div
//                             key={approachingSectionKey} // Key ensures re-animation if section changes
//                             variants={warningVariants}
//                             initial="hidden"
//                             animate="visible"
//                             exit="exit"
...
//             {/* --- UI Layer (z-60) --- (No changes here, components handle updated data) */}
//              {currentSectionKey !== 'intro' && currentSectionKey !== 'about' && currentSectionKey !== 'projects' && ( // Hide Banner for special sections
//                  <SectionTitleBanner key={currentSectionKey} title={currentSectionData?.title} icon={currentSectionData?.icon} isVisible={!!currentSectionData} />
//              )}

//             {currentSectionData?.type === 'list' && ( // Sequential Item Display renders the updated lists
//                  <SequentialItemDisplay key={currentSectionKey + "-items"} items={currentSectionData.items} visibleCount={visibleItemCount} onItemClick={() => {}} />
//              )}
...
//             {currentSectionData?.content && currentSectionKey !== 'intro' && currentSectionKey !== 'about' && ( // Bottom Left Content (e.g., Contact)
//                  <div className="absolute bottom-5 left-5 max-w-sm bg-black/70 backdrop-blur-sm p-3 rounded-lg text-white shadow-lg z-60 border border-white/10 text-xs">
//                      {/* Use whitespace-pre-wrap to respect newlines in content */}
//                      <p className="whitespace-pre-wrap">{currentSectionData.content}</p>
//                  </div>
//             )}
...
//     // --- Portfolio Data (UPDATED with Contact Details) ---
//     const sections = {
//         'intro': {
//             title: 'Welcome!', icon: <PlayCircle size={20} className="text-teal-300" />,
//             contentTitle: "Aadarsh Raj", contentSubtitle: "Interactive Portfolio", contentAction: "Run → to Explore", // Changed 'Drive' to 'Run'
//         },
//         'about': {
//             title: 'About Me', icon: <User size={20} className="text-sky-300" />,
...
//             title: 'Education', icon: <GraduationCap size={20} className="text-rose-300" />, type: 'list',
//             items: [
//                 { id: 'ed1', label: 'B.Tech CSE', description: 'Lovely Professional University (2023-2026). Focus on AI & Software Engg.', icon: <BookOpen size={18} className="text-rose-400"/> },
//                 { id: 'ed2', label: 'Diploma CSE', description: 'Lovely Professional University (2020-2023). CGPA: 8.5+. Key Courses: DSA, DBMS, OS.', icon: <GraduationCap size={18} className="text-pink-400"/> },
//                 { id: 'ed3', label: 'Matriculation (10th)', description: 'Angel High School (CBSE Board, 2020). ', icon: <GraduationCap size={18} className="text-fuchsia-400"/> }
//             ]
//         },
...
//             details: [
//                 { id: 'email', label: 'aadarshraj1516@gmail.com', href: 'mailto:aadarshraj1516@gmail.com', icon: <Mail size={18} className="text-purple-400"/> },
//                 { id: 'linkedin', label: 'Aadarsh Raj', href: 'https://linkedin.com/in/aadarsh-raj-dev', icon: <Linkedin size={18} className="text-blue-400"/> },
//                 { id: 'github', label: 'aadarshraj1516', href: 'https://github.com/aadarshraj1516', icon: <Github size={18} className="text-gray-400"/> },
//                 { id: 'phone', label: '+91-XXXXXXXXXX', href: 'tel:+91XXXXXXXXXX', icon: <Phone size={18} className="text-green-400"/> }, // Replace X's with your number
//             ]
//         },
//     };
...
//     const sectionKeys = Object.keys(sections);
//     const numSections = sectionKeys.length;
//     const sectionsToWarnAbout = ['skills', 'experience', 'projects', 'awards', 'certifications', 'education', 'contact']; // Keep as is
...
//                 {/*
//                     ** IMPORTANT **
//                     Replace ' /dinosaur-running.gif ' with the actual path to your dinosaur image.
//                     Make sure the image is in your 'public' folder or served correctly.
//                     Use an animated GIF for running animation.
//                 */}
//                 <img
//                     src="/running.gif" // <-- REPLACE THIS PATH
//                     alt="Player Dinosaur"
...
//             {/* --- UI Layer (z-60) --- (Contact section rendering updated) */}
//              {currentSectionKey !== 'intro' && currentSectionKey !== 'about' && currentSectionKey !== 'projects' && currentSectionData?.type !== 'contact' && ( // Hide Banner for special sections & contact
//                  <SectionTitleBanner key={currentSectionKey} title={currentSectionData?.title} icon={currentSectionData?.icon} isVisible={!!currentSectionData} />
//              )}
...
//                 {/* ... (keep coin count and controls toggle) */}
//                 <div className="flex items-center gap-2 bg-black/70 backdrop-blur-sm p-2 px-4 rounded-lg text-white shadow-xl border border-white/10"> <Diamond size={18} className="text-yellow-400 filter drop-shadow animate-pulse" /> <span className="text-lg font-bold text-shadow-sm">{coinCount}</span> </div>
//                 <AnimatePresence> {showControls && ( <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.2 }} className="bg-black/70 backdrop-blur-sm p-3 rounded-lg text-white text-xs shadow-xl border border-white/10 leading-relaxed w-max"> <h3 className="font-bold mb-1 uppercase text-shadow-sm text-center">Controls</h3> <p>← → / A D: Move</p> <p>↑ / Space: Jump</p> <p>C: Toggle</p> </motion.div> )} </AnimatePresence>
//             </div>

//             {/* Contact Section Display (Improved UI) */}
...
//                                     title={item.href} // Tooltip for long links
//                                 >
//                                     {item.label}
//                                 </a>
//                             </motion.li>
...
export default function PortfolioCarGame() {
    ...
    const sections = {
        ...
        'resume': {
            ...
            resumeUrl: '/apurvaResume.pdf', // Path relative to the public folder
    ...
    const sectionKeys = Object.keys(sections);
    const numSections = sectionKeys.length;
    const sectionsToWarnAbout = ['skills', 'experience', 'projects', 'awards', 'certifications', 'education', 'resume', 'contact']; // Added 'resume'
                 ...
                 >
                     ...
                     <div className="text-center"> {/* Center the link/button */}
                         <a
                             ...
                             download="Aadarsh_Raj_Resume.pdf" // Suggest a filename (change if needed)
